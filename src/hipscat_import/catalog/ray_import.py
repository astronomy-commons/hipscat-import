...
import hipscat_import.catalog.ray_map_reduce as rmr
...
def _map_pixels(args):
    raw_histogram = resume.read_histogram(args.tmp_path, args.mapping_healpix_order)
    if resume.is_mapping_done(args.tmp_path):
        return raw_histogram

    raw_histogram = db.from_sequence(
        args.input_paths,
        partition_size=1
    ).reduction(
        partial(
...
def _split_pixels(args, alignment):
    """Generate a raw histogram of object counts in each healpix pixel"""
    ...
    for i, file_path in enumerate(args.input_paths):
        split_pix_dict["input_file"].append(file_path)
        split_pix_dict["shard_suffix"].append(i)

    split_pix_df = dd.from_pandas(
        pd.DataFrame(
            split_pix_dict,
            columns=list(split_pix_dict.keys())
        ).reset_index(drop=True),
        npartitions=len(args.input_paths)
    )

    result = split_pix_df.map_partitions(
        rmr.split_pixels,
        file_reader=args.file_reader,
        filter_function=args.filter_function,
        highest_order=args.mapping_healpix_order,
        ...
        alignment=alignment,
        meta=pd.DataFrame({"result":[]}, columns=["result"])
    ).compute()
...
def _reduce_pixels(args, destination_pixel_map):
    ...
    for destination_pixel, source_pixels in destination_pixel_map.items():
        ...
        if destination_pixel_key in reduced_keys:
            continue
        reduce_pix_dict["key"].append(f"{destination_pixel.order}_{destination_pixel.pixel}")
        reduce_pix_dict["destination_pixel_order"].append(destination_pixel.order)
        reduce_pix_dict["destination_pixel_number"].append(destination_pixel.pixel)
        reduce_pix_dict["destination_pixel_size"].append(source_pixels[0])
        nparts += 1


    reduce_pix_df = dd.from_pandas(
        pd.DataFrame(
            reduce_pix_dict,
            columns=list(reduce_pix_dict.keys())
        ).reset_index(drop=True),
        npartitions=nparts
    )

    result = reduce_pix_df.map_partitions(
        rmr.reduce_pixel_shards,
        cache_path=args.tmp_path,
...
def run_without_client(args, client):
    """Run catalog creation pipeline."""
    if not args:
        raise ValueError("args is required and should be type ImportArguments")
    if not isinstance(args, ImportArguments):
    ...
    #map pixels
    # create a dask dataframe that encapsulates all the arguments 
    # of map_pixels, then write a custom mr.map_pixels function with 
    raw_histogram = _map_pixels(args)
    ...
    if not args.debug_stats_only:
        #alignment_future = client.scatter(alignment)
        _split_pixels(args, alignment)
        _reduce_pixels(args, destination_pixel_map)

    return "Happy Happy boy man :)"

    #result2 = _map_pixels(args, client)

    #print(type(result1), type(result2))
    
    #assert result1 == result2, "Wah!"